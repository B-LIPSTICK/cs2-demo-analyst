/**
 * Web 版 demo 解析核心：浏览器内用 deadem 解析 .dem（Web Streams，无 Node 依赖）。
 * 事件适配逻辑与 src/main/services/parser.ts 一致（CS2 1.41: round_prestart/round_officially_ended 等）。
 */
import {
  InterceptorStage,
  MessagePacketType,
...
export interface ParseResult {
  ...
  hasVoice: boolean
  voiceSec: number
  voiceMsgs: { tick: number; entity: number; xuid?: string; voiceData: string }[]
  teamT?: string
  teamCT?: string
  scoreT: number
  scoreCT: number
...
function bytesToBase64(data: string | Uint8Array): string {
...
export async function parseDemoWeb(
  ...
  const counted = new TransformStream<Uint8Array, Uint8Array>({
    transform(chunk, controller) {
      ...
      if (read % (totalBytes / 40) < 64 * 1024 || read >= totalBytes) {
        onProgress?.({ bytes: read, total: totalBytes })
      }
  ...
  const chat: RawChat[] = []
  const voiceMsgs: ParseResult['voiceMsgs'] = []
  ...
  let currentRound: RoundAcc = { roundNum: 1, startTick: 0, winner: 'none', endType: 'unknown', kills: [] }
  ...
  const startRound = (tick: number) => {
    if (roundsStarted) closeRound(tick)
    roundsStarted = true
    currentRound = { roundNum: rounds.length + 1, startTick: tick, winner: 'none', endType: 'unknown', kills: [] }
  }
  ...
  const handleGameEvent = (eventId: number, keys: (EventKey | null)[], tick: number) => {
    ...
    switch (name) {
      case 'round_prestart': startRound(tick); break
      case 'round_officially_ended':
        if (currentRound.endTick === undefined) currentRound.endTick = tick
        break
      case 'begin_new_match': lastBeginNewMatchTick = tick; break
      case 'round_announce_match_start': lastMatchStartTick = tick; break
      case 'player_team': {
        const uid = int(ev.userid)
        const teamN = int(ev.team)
        const slot = playersByUserid.get(uid)
        if (slot && (teamN === 2 || teamN === 3)) slot.team = teamN === 2 ? 'T' : 'CT'
        break
      }
      case 'player_death': {
      ...
      case 'bomb_planted': currentRound.bombPlantedTick = tick; break
      case 'bomb_defused': currentRound.bombDefusedTick = tick; break
      case 'bomb_exploded': currentRound.bombExplodedTick = tick; break
      default: break
    }
  }
  ...
  parser.registerPostInterceptor(InterceptorStage.DEMO_PACKET, async (demoPacket) => {
    ...
    if (!roundsStarted && firstTick !== Infinity) currentRound.startTick = firstTick
  ...
  parser.registerPostInterceptor(InterceptorStage.MESSAGE_PACKET, async (demoPacket, messagePacket) => {
    ...
    switch (messagePacket.type) {
      ...
      case MessagePacketType.GE_SOURCE1_LEGACY_GAME_EVENT_LIST: {
        const data = messagePacket.data as { descriptors: { eventid: number; name?: string; keys: { name: string }[] }[] }
      ...
      case MessagePacketType.SVC_VOICE_DATA: {
        const d = messagePacket.data as {
          audio?: { voiceData?: string | Uint8Array }
          entity?: number
          xuid?: { low: number; high: number }
        }
        const raw = d?.audio?.voiceData
        if (raw && (raw instanceof Uint8Array ? raw.length > 0 : raw.length > 0)) {
          const xuid = d.xuid ? (BigInt(d.xuid.high) << 32n) + BigInt(d.xuid.low) : undefined
          voiceMsgs.push({
            tick,
            entity: d.entity ?? -1,
            xuid: xuid?.toString(),
            // protobuf bytes 字段在 deadem 中为 Uint8Array，统一转 base64 便于传输/序列化
            voiceData: bytesToBase64(raw)
          })
        }
  ...
  await parser.parse(stream.pipeThrough(counted))
  ...
  try {
    ...
    for (const team of teams) {
      ...
      if (num === 2) { teamT = name; scoreT = score }
      if (num === 3) { teamCT = name; scoreCT = score }
    }
    const controllers = demo.getEntitiesByClassName('CCSPlayerController')
    const nameToTeam = new Map<string, TeamSide>()
    for (const c of controllers) {
      const nm = c.getField('m_iszPlayerName')
      const tn = c.getField('m_iTeamNum')
      if (typeof nm === 'string' && (tn === 2 || tn === 3)) nameToTeam.set(nm, tn === 2 ? 'T' : 'CT')
    }
    for (const slot of playersByUserid.values()) {
      const team = nameToTeam.get(slot.name)
  ...
  const realRounds = rounds
    .filter(
      (r) => (minStartTick < 0 || r.startTick >= minStartTick) && (r.endTick ?? lastTick) - r.startTick >= 64 * 5
    )
  ...
  return {
    ...
    chat: chat.sort((a, b) => a.tick - b.tick).map((c) => ({ ...c, timeSec: c.tick / TICK_RATE })),
    hasVoice: voiceMsgs.length > 0,
