...
- **用户需求与痛点诊断**:
  ...
  2. **本地语音转写模型下载与进度条核查**: 用户要求确认本地语音转写模型（Whisper ggml 与 whisper-cli / csgove）下载是否好使、是否有进度条显示等。
- **深度排查与根因分析**:
  1. **对局切换**: `RoundFilterBar` 之前仅依赖滚轮 `onWheel`，视觉上缺少可视化的左右翻页按钮，普通鼠标用户无法直观点击滑动；且切换单回合时缺少直接的「上一局 / 下一局」快速步进器；
  2. **本地模型下载进度条失灵（重大发现）**:
     - 在 `SettingsPage.tsx` 中，引擎列表 `ENGINE_ROWS` 定义为 `kind: 'whisper'`, `'model-base'`, `'model-small'`, `'model-medium'`；
     - 但在 `engines.ts` 中，`ensureWhisperCli` 广播的进度标识为 `what: 'whisper-cli'`，`ensureWhisperModel` 广播的为 `what: 'model-ggml-base.bin'`；
     - 字符串完全不匹配，导致设置面板中点击下载任何 Whisper 模型时，**进度条从未渲染出来**，误以为没有进度或卡死；
  3. **转写页面静默下载问题**:
     - 在 `asr.ts` 的 `transcribeLocal` 中，原代码直接 `await ensureWhisperCli(() => {})` 和 `await ensureWhisperModel(model, () => {})`，传入空回调；
     - 用户在转写页点击转写时，百兆至千兆的模型在后台静默下载，前端页面没有任何下载阶段和进度百分比显示，造成长时间假死假象；
  4. **下载中断损坏与文件完整性缺陷**:
     - `download.ts` 原本直接向目标文件写入，若用户网络波动或强退，磁盘会遗留几兆的半拉损坏文件；
     - `engines.ts` 原本的 `exists` 仅判断 `fs.access`，导致残留的损坏文件被误认为「已安装」，后续每次调用 whisper 均会报 magic number 错误或崩溃；
     - `download.ts` 对大型文件使用固定 20 秒请求超时，长耗时下载容易被错误超时中断。
- **重构与优化实施**:
  1. **击杀记录对局切换交互全方位升级**:
     - **左右可视化步进导航按钮 (`<` / `>`)**: 在 `RoundFilterBar` 左右两端嵌入专用的毛玻璃步进箭头按钮，普通鼠标用户单机即可平滑步进滑动视口（±150px）；
     - **边界感知与防误触**: 自动监听滚动位置，滑至最左时 `<` 自动禁用并淡化，滑至最右时 `>` 自动禁用并淡化；支持 ResizeObserver 响应窗口变化；
     - **全轴向滚轮兼容**: 鼠标悬浮在筛选条上时，同时支持普通鼠标纵向滚轮（`deltaY` 自动转为横向平移）以及高级鼠标横向滚轮（`deltaX`）；
     - **击杀流头部增加「上一局 / 下一局」快切按钮**: 击杀明细面板标题右侧增加 `‹ 上一局` 与 `下一局 ›` 步进器，无需滚动列表即可一键连续查看相邻对局击杀；
     - **深浅双色全景精修**: 在暗夜与浅色模式下分别匹配磨砂亚克力电竞质感，离屏截图严格校验。
  2. **本地语音转写模型下载与进度链路全线修复**:
     - **原子化下载与空闲超时改造 (`download.ts`)**:
       - 所有下载均写入 `.tmp` 临时文件，仅在校验完整下载完成后原子覆盖，杜绝残留半拉文件；
       - 收到响应头后清除连接超时，改为 30s Socket 空闲超时，支持超大模型长时间顺畅传输；
       - 进度事件采用时间节流（~100ms），提供高帧率（10Hz）平滑进度。
     - **模型完整性自动校验与坏件自愈 (`engines.ts`)**:
       - 淘汰裸 `fs.access`，增加 `isValidFile` 阈值检测（csgove/whisper > 100KB，base/small/medium 模型需达到预定体积的 85% 以上）；
       - 遇到 0 字节或半截损坏文件自动清理并触发重新下载，保证调用绝对稳健；
       - 统一进度标识符（`whisper`、`model-base` 等），彻底打通设置页与主进程进度事件映射。
     - **转写前置下载进度广播 (`asr.ts`)**:
       - 在转写执行前先统一检测引擎与模型，若未下载则实时广播 `download-engine` 进度事件，携带已下载/总 MB 与实时百分比；
       - `csgove` 语音提取工具下载同样实时汇报具体下载量。
     - **转写页与设置页 UI 进度条可视化 (`TranscriptPage.tsx` & `SettingsPage.tsx`)**:
       - 转写页在下载模型阶段展示独立的进度卡片，清晰呈现如「正在下载语音模型 (ggml-base.bin): 45.2MB / 142.0MB (32%)」；
       - 设置页引擎管理列表在下载时实时显示精准百分比、已下载 MB / 总 MB，以及双色渐变平滑动画进度条，下载期间禁用重复点击。
- **验证与产物构建**:
  - `npm run typecheck` ✓（0 报错）
  - `npm run build` ✓
  - 产物构建完成：
    - `dist/CS2-Demo-Analyst-1.0.0-win64-portable.zip` (147 MB)
    - `dist/CS2-Demo-Analyst-1.0.0-setup.exe` (92 MB)

## 本轮二十二（修复选手弹窗受对局筛选影响变成单局/空数据、击杀明细时间倒序改为正序）✅
- **用户反馈诊断**:
  - 用户反馈：「击杀明细反了，从后往前了，并且在我在击杀记录里面选择特定对局的时候，点击 选手数据，他就没有数据（？可能变成这一局的击杀记录了？我觉得没有必要，一直显示全局击杀记录就可以）」。
  - **根因确认**:
    1. **击杀明细时间倒序**: `PlayerModal` 中的击杀列表原本使用了 `[...myKills].sort((a, b) => b.tick - a.tick)`，导致时间戳从后往前排（如截图所示，第 22 回合 37:03 的击杀排在最顶上，而开局第 1 回合的击杀沉在最底端），不符合通常时间线正序从开局回放的习惯；
    2. **选手弹窗数据被单局过滤导致空数据**: `DemoDetailPage` 原先使用 `const kills = round ? round.kills : allKills`，在用户点击了特定回合筛选（如 R3）后，将仅包含单回合击杀的 `kills` 透传给了 `<PlayerModal kills={kills} />`。如果该选手在该回合未击杀或死亡，选手弹窗内的武器统计与击杀明细直接归零变为空白。
- **修复方案**:
  1. **选手数据弹窗锁定全局击杀（`allKills`）**:
     - `DemoDetailPage` 中将击杀流专用的局部过滤 `feedKills` 与全场全局击杀 `allKills` 明确分离；
     - `<PlayerModal kills={allKills} />` 永远接收整场比赛的全部击杀数据，无论主页面当前在看哪一回合，选手弹窗始终完整展示该选手的整场击杀总数、全武器分布与完整击杀流水，彻底解决误变单局或空数据问题；
  2. **击杀明细改为时间正序（从前到后）**:
     - `PlayerModal` 击杀明细排序调整为 `[...myKills].sort((a, b) => a.tick - b.tick)`；
     - 击杀流列表从开局第 1 回合（R1）第一杀自上而下顺次排列至最终回合，清晰还原比赛击杀时间脉络。
- **验证与打包**:
  - `npm run typecheck` ✓
  - `npm run build` ✓
  - 重新打包 portable zip 与 setup exe。

## 本轮二十一（彻底修复击杀记录对局筛选横向溢出穿模、增加平滑滚轮横向滑动与居中高亮）✅
- **用户反馈诊断**:
  - 用户反馈：「击杀记录这里对局穿模了」并附带截图。
  - **根因确认**:
    1. 击杀流卡片顶部原本使用分段控件 `.seg` 展示对局回合切换（全部、1、2...24）。`.seg` 为 `display: inline-flex`，未设置最大宽度与换行/横向溢出处理；
    2. 当一场比赛有 20~24 回合以上时，`.seg` 宽度达到 750px+，而击杀流面板在栅格布局中只有约 400~500px 宽，导致对局按钮横向穿透卡片边框、直接「穿模」飞入右侧的「选手数据」卡片；
    3. 同时 `.panel` 未加 `overflow: hidden`，导致任何越界子元素直接外露破坏界面布局。
- **重构与优化实施**:
  1. **打造专属 `RoundFilterBar` 组件**:
     - 将原本的普通 `seg` 升级为专用的横向平滑滚动筛选栏（`round-filter-wrap` + `round-filter-scroll`）；
     - 宽度严格限制在 100% 容器内部，启用 `overflow-x: auto`，并隐藏丑陋的原生滚动条（`-webkit-scrollbar: none` / `scrollbar-width: none`）；
     - **原生鼠标滚轮横向交互**: 监听 `onWheel`，鼠标置于对局按钮上方时滚轮上下滑动直接平滑水平横移；
     - **自动居中滚动**: 点击任意回合或通过上方时间线切回合时，选中回合自动平滑滚动至视口中央可见（`scrollIntoView`）；
     - **胜负阵营指示微标**: 每个回合编号下方内置精致阵营微点（琥珀金 T / 冰霜蓝 CT），在筛选的同时直观看到当局胜者；
  2. **面板边界严密加固**:
     - `.panel` 全局追加 `overflow: hidden`，彻底杜绝任何子组件、图表或长内容溢出穿模破损相邻卡片。
- **验证与打包**:
  - `npm run typecheck` ✓
  - `npm run build` ✓
  - Electron 离屏真实渲染截图比对验证（`verify_fix.png` / `test_hidden_scroll.png`）：各回合按钮紧密内嵌，右侧选手数据面板边界严丝合缝，零穿模；
  - 便携版 zip 与安装包 exe 重新打包更新。

## 本轮二十（击杀流视觉锐化重构：彻底清除雾化模糊、眼睛图标全尺寸矢量化、黑夜与亮色模式高对比度优化）✅
- **问题反馈定位**:
  - 用户反馈：「还是难看，现在感觉雾蒙蒙的看不清，眼睛图标小，黑夜模式看不清，爆头雾蒙蒙的」。
  - 排查确认：
    1. 原 CSS 对所有击杀流勋章（爆头、烟、闪、穿墙等）应用了 `filter: drop-shadow(0 0 3px ...)`，且容器设置了 `backdrop-filter: blur(8px)`，导致 14px 的小矢量在渲染时被 3px 模糊阴影包围，红骷髅眼孔糊死呈现「红色雾蒙蒙发光团」；
    2. 致盲反杀使用系统 emoji `🕶️`，在 Windows DirectWrite 下上下填充大量空白字距，实际图像仅约 8px 且发黑发紫，在深色和浅色背景下均难以看清；
    3. 黑夜模式下击杀行背景过于灰暗，选手姓名带有 12px 模糊外发光文字阴影，字符发虚发散。
- **重构与优化实施**:
  1. **彻底消除「雾蒙蒙」感**:
     - 移除全部 `filter: drop-shadow`，移除 `.wp` 武器胶囊容器上的 `backdrop-filter: blur(8px)`；
     - 爆头红骷髅升级为 CS2 / CS:GO 经典大眼窝、深鼻腔、双层颌骨牙齿的高清晰度矢量 SVG，15px 尺寸下眼骨分明，纯鲜红色（夜间 `#ff3344` / 日间 `#dc2626`）；
  2. **致盲反杀「眼睛图标」全尺寸矢量化**:
     - 淘汰系统 emoji `🕶️`，定制 16px×16px 矢量 SVG 划线眼睛（Blinded Eye，大轮廓眼形 + 实心瞳孔 + 鲜明对角划线）；
     - 夜间模式高亮天蓝 `#38bdf8`，亮色模式深湛蓝 `#0284c7`，尺寸与其它勋章严格对齐，清晰显眼；
  3. **击杀流其余勋章全套矢量重绘**:
     - 闪光助攻：黄色纯量闪电矢量（`#facc15`）；
     - 穿墙击杀：双层障碍穿越贯穿箭头矢量（`#fb923c`）；
     - 盲狙击杀：十字准星划线矢量（`#34d399`）；
     - 穿烟击杀：规整烟雾云朵矢量（夜间 `#e2e8f0` / 日间 `#64748b`）；
  4. **黑夜与日间模式对比度全面升级**:
     - 移除选手名字的 12px 模糊文字阴影，恢复平整锐利字形；T 阵营使用亮琥珀金 `#fbbf24`（日间 `#d97706`），CT 阵营使用高纯度天蓝 `#38bdf8`（日间 `#0284c7`）；
     - 击杀行与武器胶囊重调明暗阶梯，暗色模式下行卡片底色 `rgba(255,255,255,0.035)` + 边框 `0.08`，武器胶囊 `rgba(255,255,255,0.07)` + 边框 `0.18`；
     - `PlayerModal` 击杀列表中修复日间模式下武器文字及受害者文字白底隐形问题。
- **验证与打包**:
  - `npm run typecheck` ✓
  - `npm run build` ✓
  - 经 Electron 离屏真实渲染截图比对验证，黑夜模式与日间模式均清晰锐利、无模糊晕影；
  - `npm run dist:zip`（产出便携版 zip 147MB）及 `npx electron-builder --win nsis` 安装包更新。

## 本轮二十（全应用下拉栏统一且防透底、离线模型链接一键复制/打开、AI 页面去土味极净化、B站空间更新与窗口控制手感升级）✅
- **功能一：本地引擎与模型离线安装弹窗支持文本选中与一键复制/打开**:
  1. **支持鼠标选中文本**: `Modal` 容器显式配置 `userSelect: 'text'` 与 `backdrop-filter: blur(4px)`，彻底解决应用全局禁止文本选择导致弹窗内无法划选复制的体验缺陷。
  2. **下载源独立卡片化**: 将 HuggingFace 官方源、HF-Mirror 国内高速镜像、whisper.cpp Releases、csgove Releases 全部独立卡片化呈现，附带用途说明与完整 URL。
  3. **快捷交互按钮**: 每个下载源卡片右侧直接提供「复制」按钮（一键写入剪贴板并弹出 Toast）与「访问」按钮（调用系统默认浏览器直达页面），免去手动输入烦恼。
- **功能二：全应用下拉栏统一风格与彻底消除透底**:
  1. **实体背景消灭透底**: 修复 `hud.css` 中 `.custom-select-menu` 引用未定义变量 `--glass-card-deep` 导致在 Chromium 中回退为全透明、文字与底色严重透出的 Bug；改为深色实体底 `#18191f` 与浅色实体底 `#ffffff`，配合立体悬浮投影（`box-shadow: 0 16px 40px rgba(0,0,0,0.75)`）与微边框。
  2. **AI 页面接入全局 CustomSelect**: 彻底废弃 `AiPage` 中私有的旧 `AiSelect`，全量换入 `CustomSelect`。资料库排序、语音转写选择、设置页模型语言选择、AI 页面会话与 Demo 选择全面达成一致的亚克力质感。
- **功能三：AI 战术助手界面彻底去土味与极净化**:
  1. **移除土味大图标与冗长文案**: 移除中间巨大的星星图标（`IcSpark 30`）、移除「向 AI 提问，秒懂这局比赛」粗体大字与冗长的副标题说明。
  2. **全面清理 Emoji 装饰**: 移除四个快捷模拟提问中的所有 emoji（🎬、😤、📈、👤）；移除顶部会话选择框中的 `💬`、Demo 选择框中的 `🎬`。
  3. **居中优雅排版**: 快捷问题 chip 自适应居中展示，干净利落。
  4. **移除底部标签栏与顶部轻量化语音标记**: 彻底移除底部「数据基于本局解析...答案中的时间与回合...」标签栏；转为在顶部 Demo 下拉框旁轻量呈现「• 有语音」/「无语音」胶囊标签。
- **功能四：右上角最小化/全屏/关闭按键交互手感升级**:
  1. **扩大判定热区**: 从此前 12px 极小微型按钮升级为 26px × 26px 灵敏判定热区容器，内嵌 12.5px 苹果红黄绿立体小圆点。
  2. **防滑防误拖拽**: 彻底消除鼠标极易点击偏移误触到标题栏拖拽区域（`-webkit-app-region: drag`）导致窗口晃动的误触问题，悬停带微放大与发光效果。
- **功能五：项目主页与作者链接更新**:
  1. **左下角 B 站链接更换**: 更新为用户做项目的官方主页：`https://space.bilibili.com/3632307015518585`（B-LIPSTICK 个人空间）。
- **质量验证**:
  - `npm run typecheck` 0 报错通过；
  - `npm run smoke` 运行通过，渲染进程加载 OK；
  - Electron 无头渲染暗色截图验证，确认 UI 纯净、文字对齐、下拉菜单实底不透；
  - `npm run dist:zip` 成功编译打包便携包 `CS2-Demo-Analyst-1.0.0-win64-portable.zip` (147 MB)。
...
- **功能四：夜间模式击杀流重构与高辨识度竞技对比度增强**:
  1. **阵营色提亮与微光氛围**: 暗黑模式下将低沉灰暗的 `--t` (`#c9a86a`) 升级为亮眼电竞琥珀金 `#f5a623`，`--ct` (`#7ba3c9`) 升级为清爽高辨识冰霜蓝 `#5dade2`，并在文字外层追加微光光晕（`text-shadow: 0 0 12px`），彻底解决暗黑背景下名字发暗看不清的问题。
  2. **动作向心紧凑排版（Align to Weapon）**: 废弃两端分散的生硬对齐，采用官方 CS2/HLTV 风格的向心对齐布局——击杀者右对齐紧挨武器胶囊左侧、受害者左对齐紧随武器胶囊右侧，消除两者之间巨大空白虚位，视线无需跳跃即可一眼捕捉「击杀者 [武器+勋章] 受害者」。
  3. **立体武器胶囊 (Weapon Capsule)**: 武器名由暗灰升级为高光纯白 `#f8fafc`，容器采用精致磨砂高透亚克力胶囊（`rgba(255,255,255,0.08)` + `border 0.16` + `backdrop-filter: blur`），鼠标悬停灵动发光。
  4. **高对比度击杀勋章**:
     - 爆头红骷髅：升级为霓虹猩红 `#ff453a` + 3px 柔和外发光；
     - 穿烟云朵：彻底告别暗灰隐形，改为明亮银白 `#f1f5f9` + 柔光阴影；
     - 闪光助攻：金黄高亮雷电 ⚡ + 外发光；
     - 穿墙/盲狙/致盲反杀勋章全套高对比度滤镜增强。
  5. **整行玻璃质感与交互**: 每一条击杀流包裹一层超轻量亚克力卡片底色（`rgba(255,255,255,0.025)`），Hover 时浮起发光（`-1px` 悬停动效 + 投影），回合数转为精致紧凑胶囊标签。
- **验证与交付**:
