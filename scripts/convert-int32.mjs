// 正确的 int32→int16 转换（取高 16 位）与 app 的低 16 位对比
import { readFile, writeFile } from 'node:fs/promises'
const src = process.argv[2]
const outFile = process.argv[3]
const mode = process.argv[4] ?? 'high' // high | low | scale
...
for (let i = 0; i < nSamples; i++) {
  ...
  let s16
  if (mode === 'high') s16 = v >> 16 // 取高 16 位（正确）
  else if (mode === 'low') s16 = v & 0xffff // 取低 16 位（app 的 bug）
  else s16 = Math.round((v / 2147483647) * 32767) // 浮点缩放
  out.writeInt16LE(s16, 44 + i * 2)
}
await writeFile(outFile, out)
console.log(`wrote ${outFile} (mode=${mode}, ${nSamples} samples)`)
